...
const reducer = (state: CartStateType, action: ReducerAction): CartStateType => {
  switch (action.type) {
    case REDUCER_ACTION_TYPE.ADD: {
      if (!action.payload) {
        throw new Error(`${action.payload} missing in ${REDUCER_ACTION_TYPE.ADD} action`);
      }
      ...
      return { ...state, cart: [...filteredCart, { sku, name, quantity, price, img }] };
    }

    case REDUCER_ACTION_TYPE.REMOVE: {
      if (!action.payload) {
        throw new Error(`${action.payload} missing in ${REDUCER_ACTION_TYPE.REMOVE} action`);
      }
    ...
    case REDUCER_ACTION_TYPE.QUANTITY: {
      if (!action.payload) {
        throw new Error(`${action.payload} missing in ${REDUCER_ACTION_TYPE.QUANTITY} action`);
      }
